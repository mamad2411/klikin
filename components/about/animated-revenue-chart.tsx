...
import { TrendingUp, ShoppingBag, UtensilsCrossed, Settings, Shirt } from "lucide-react"

const revenueCategories = [
  { name: "Retail & Kelontong", icon: ShoppingBag, color: "#3b82f6" },
  { name: "Kuliner (F&B)", icon: UtensilsCrossed, color: "#10b981" },
  { name: "Jasa & Servis", icon: Settings, color: "#8b5cf6" },
  { name: "Fashion & Lifestyle", icon: Shirt, color: "#f59e0b" },
]
...
function generateInitialData() {
  return revenueCategories.map((cat) => ({
    ...cat,
    value: 350000, // Fixed value for initial SSR to avoid hydration mismatch
  }))
}
...
export function AnimatedRevenueChart() {
  ...
  // Set mounted and trigger first data update
  useEffect(() => {
    setMounted(true)
  ...
  return (
    ...
    >
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-slate-900">Pertumbuhan Mitra</h3>
        ...
        <div className="text-right">
          ...
          >
            Rp {mounted ? totalRevenue.toLocaleString("id-ID") : totalRevenue.toLocaleString("id-ID")}
          </motion.p>
          <motion.div
            key={growth}
            initial={{ opacity: 0 }}
          ...
          return (
            ...
            >
              ...
              <div className="text-right">
                ...
                >
                  Rp {mounted ? item.value.toLocaleString("id-ID") : item.value.toLocaleString("id-ID")}
                </motion.span>
